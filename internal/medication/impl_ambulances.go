...
// EditAmbulance - Updates specific ambulance details
func (this *implAmbulancesAPI) EditAmbulance(ctx *gin.Context) {
	value, exists := ctx.Get("db_service")
	if !exists {
	...
	ambulance.Id = ambulanceId // Ensure the ambulance ID is set correctly
	err = db.UpdateDocument(ctx, ambulanceId, &ambulance)
	switch err {
	case nil:
		ctx.JSON(http.StatusOK, ambulance)
	case db_service.ErrNotFound:
...
// GetAmbulance - Retrieves specific ambulance details
